...
// ============================================================
// Reads live data from your private Apps Script backend, which
// in turn reads the Google Sheet + Photos folder. Nothing about
// people/photos/tags is hardcoded here — it's all read
// dynamically so the Sheet remains the single source of truth.
// ============================================================
...
function displayName(rec) {
  ...
  // Two different people with two different last names — e.g. "Mike Beegen & Lisa Anne Tumminia"
  if (f["First 2"] && f["Last 2"] && f["Last 2"] !== f["Last 1"]) {
    const person1 = `${f["First 1"] || ""} ${f["Last 1"] || ""}`.trim();
    const person2 = `${f["First 2"]} ${f["Last 2"]}`.trim();
    return `${prefix}${person1} & ${person2}${suffix}`.trim();
  }

  // One person with two last names — e.g. "Shannon Kiniry Paria" or "Virginia Tupper Pennington"
  // Or a couple sharing one last name — e.g. "Mike & Jen Winkelmann"
  let name = prefix + (f["First 1"] || "");
